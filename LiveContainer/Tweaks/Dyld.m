...
static uint32_t (*orig_nw_interface_get_index)(nw_interface_t interface);
...
static NSArray *(*orig_NWPath_interfaces_objc)(id self, SEL _cmd);
static id (*orig_NWPath_interface_objc)(id self, SEL _cmd);
static id (*orig_NWPath_connectedInterface_objc)(id self, SEL _cmd);
static id (*orig_NWPath_delegateInterface_objc)(id self, SEL _cmd);
...
static uint32_t hook_nw_interface_get_index(nw_interface_t interface);
...
static NSArray *hook_NWPath_interfaces_objc(id self, SEL _cmd);
static id hook_NWPath_interface_objc(id self, SEL _cmd);
static id hook_NWPath_connectedInterface_objc(id self, SEL _cmd);
static id hook_NWPath_delegateInterface_objc(id self, SEL _cmd);
...
static BOOL lc_shouldEnableSwiftNetworkImportRebind(void);
...
void* hook_dlsym(void * __handle, const char * __symbol) {
    ...
    if (__symbol &&
        (strcmp(__symbol, "nw_interface_get_name") == 0 ||
         strcmp(__symbol, "_nw_interface_get_name") == 0 ||
         strcmp(__symbol, "__nw_interface_get_name") == 0)) {
        lc_resolveNetworkSymbolPointers();
        static BOOL loggedNWGetName = NO;
        if (!loggedNWGetName) {
            loggedNWGetName = YES;
            NSLog(@"[LC] 🌐 dlsym override for %s -> hook_nw_interface_get_name", __symbol);
        }
        return (void *)hook_nw_interface_get_name;
    }

    if (__symbol &&
        (strcmp(__symbol, "nw_interface_get_index") == 0 ||
         strcmp(__symbol, "_nw_interface_get_index") == 0 ||
         strcmp(__symbol, "__nw_interface_get_index") == 0)) {
        lc_resolveNetworkSymbolPointers();
        static BOOL loggedNWGetIndex = NO;
        if (!loggedNWGetIndex) {
            loggedNWGetIndex = YES;
            NSLog(@"[LC] 🌐 dlsym override for %s -> hook_nw_interface_get_index", __symbol);
        }
        return (void *)hook_nw_interface_get_index;
    }

    if (__symbol &&
        (strcmp(__symbol, "nw_interface_get_type") == 0 ||
         strcmp(__symbol, "_nw_interface_get_type") == 0 ||
         strcmp(__symbol, "__nw_interface_get_type") == 0)) {
        lc_resolveNetworkSymbolPointers();
        static BOOL loggedNWGetType = NO;
        if (!loggedNWGetType) {
            loggedNWGetType = YES;
            NSLog(@"[LC] 🌐 dlsym override for %s -> hook_nw_interface_get_type", __symbol);
        }
        return (void *)hook_nw_interface_get_type;
    }

    if (__symbol &&
        (strcmp(__symbol, "nw_path_enumerate_interfaces") == 0 ||
         strcmp(__symbol, "_nw_path_enumerate_interfaces") == 0 ||
         strcmp(__symbol, "__nw_path_enumerate_interfaces") == 0)) {
        lc_resolveNetworkSymbolPointers();
        static BOOL loggedNWEnumerate = NO;
        if (!loggedNWEnumerate) {
            loggedNWEnumerate = YES;
            NSLog(@"[LC] 🌐 dlsym override for %s -> hook_nw_path_enumerate_interfaces", __symbol);
        }
        return (void *)hook_nw_path_enumerate_interfaces;
    }

    if (__symbol &&
        (strcmp(__symbol, "nw_path_copy_interface_with_generation") == 0 ||
         strcmp(__symbol, "_nw_path_copy_interface_with_generation") == 0 ||
         strcmp(__symbol, "__nw_path_copy_interface_with_generation") == 0)) {
        lc_resolveNetworkSymbolPointers();
        static BOOL loggedNWCopyWithGeneration = NO;
        if (!loggedNWCopyWithGeneration) {
            loggedNWCopyWithGeneration = YES;
            NSLog(@"[LC] 🌐 dlsym override for %s -> hook_nw_path_copy_interface_with_generation", __symbol);
        }
        return (void *)hook_nw_path_copy_interface_with_generation;
    }

    if (__symbol &&
        (strcmp(__symbol, "nw_interface_create_with_name") == 0 ||
         strcmp(__symbol, "_nw_interface_create_with_name") == 0)) {
        lc_resolveNetworkSymbolPointers();
        static BOOL loggedNWCreateWithName = NO;
        if (!loggedNWCreateWithName) {
            loggedNWCreateWithName = YES;
            NSLog(@"[LC] 🌐 dlsym override for %s -> hook_nw_interface_create_with_name", __symbol);
        }
        return (void *)hook_nw_interface_create_with_name;
    }

    if (__symbol &&
        (strcmp(__symbol, "nw_interface_create_with_index_and_name") == 0 ||
         strcmp(__symbol, "_nw_interface_create_with_index_and_name") == 0)) {
        lc_resolveNetworkSymbolPointers();
        static BOOL loggedNWCreateWithIndexAndName = NO;
        if (!loggedNWCreateWithIndexAndName) {
            loggedNWCreateWithIndexAndName = YES;
            NSLog(@"[LC] 🌐 dlsym override for %s -> hook_nw_interface_create_with_index_and_name", __symbol);
        }
        return (void *)hook_nw_interface_create_with_index_and_name;
    }
...
static const char *lc_findLoadedImageNameForHeader(const mach_header_u *header, intptr_t *slideOut) {
    if (slideOut) {
        *slideOut = 0;
    }
    if (!header) {
    ...
    uint32_t imageCount = _dyld_image_count();
    for (uint32_t i = 0; i < imageCount; i++) {
        const mach_header_u *currentHeader = (const mach_header_u *)_dyld_get_image_header(i);
...
static BOOL lc_rebindNWImportsForImage(const mach_header_u *header, intptr_t slide) {
    if (!header) {
        return NO;
    }

    struct rebinding nwRebindings[16];
    size_t nwRebindingCount = 0;
    nwRebindings[nwRebindingCount++] = (struct rebinding){"nw_interface_get_name", (void *)hook_nw_interface_get_name, NULL};
    nwRebindings[nwRebindingCount++] = (struct rebinding){"nw_interface_get_index", (void *)hook_nw_interface_get_index, NULL};
    nwRebindings[nwRebindingCount++] = (struct rebinding){"nw_interface_get_type", (void *)hook_nw_interface_get_type, NULL};
    nwRebindings[nwRebindingCount++] = (struct rebinding){"nw_path_enumerate_interfaces", (void *)hook_nw_path_enumerate_interfaces, NULL};
    nwRebindings[nwRebindingCount++] = (struct rebinding){"nw_path_copy_interface_with_generation", (void *)hook_nw_path_copy_interface_with_generation, NULL};
    nwRebindings[nwRebindingCount++] = (struct rebinding){"nw_interface_create_with_name", (void *)hook_nw_interface_create_with_name, NULL};
    nwRebindings[nwRebindingCount++] = (struct rebinding){"nw_interface_create_with_index_and_name", (void *)hook_nw_interface_create_with_index_and_name, NULL};
    nwRebindings[nwRebindingCount++] = (struct rebinding){"__nw_interface_get_name", (void *)hook_nw_interface_get_name, NULL};
    nwRebindings[nwRebindingCount++] = (struct rebinding){"__nw_interface_get_index", (void *)hook_nw_interface_get_index, NULL};
    nwRebindings[nwRebindingCount++] = (struct rebinding){"__nw_interface_get_type", (void *)hook_nw_interface_get_type, NULL};
    nwRebindings[nwRebindingCount++] = (struct rebinding){"__nw_path_enumerate_interfaces", (void *)hook_nw_path_enumerate_interfaces, NULL};
    nwRebindings[nwRebindingCount++] = (struct rebinding){"__nw_path_copy_interface_with_generation", (void *)hook_nw_path_copy_interface_with_generation, NULL};

    return (rebind_symbols_image((void *)header, slide, nwRebindings, nwRebindingCount) == 0);
}

static void lc_forceRebindSwiftNetworkImports(void) {
    if (!lc_shouldEnableSwiftNetworkImportRebind()) {
        return;
    }

    if (gDidApplySwiftNetworkRebind) {
        return;
    }
    ...
    if (!swiftNetworkHeader) {
        if (!gLoggedMissingSwiftNetworkImage) {
            gLoggedMissingSwiftNetworkImage = YES;
            NSLog(@"[LC] 🌐 libswiftNetwork not loaded yet; waiting for image callback");
        }
        return;
    }

    intptr_t slide = 0;
    const char *imageName = lc_findLoadedImageNameForHeader(swiftNetworkHeader, &slide);
    if (lc_rebindNWImportsForImage(swiftNetworkHeader, slide)) {
        gDidApplySwiftNetworkRebind = YES;
        NSLog(@"[LC] 🌐 rebound NW imports in %s", imageName ? imageName : "libswiftNetwork");
        return;
    }

    static BOOL loggedRebindFailure = NO;
    if (!loggedRebindFailure) {
        loggedRebindFailure = YES;
        NSLog(@"[LC] ⚠️ failed to rebind NW imports in %s", imageName ? imageName : "libswiftNetwork");
    }
}
...
static void lc_resolveNetworkSymbolPointers(void) {
    ...
    if (orig_dlsym) {
        ...
        if (!orig_nw_interface_get_index) {
            if (libnetworkHandle) {
                orig_nw_interface_get_index = (uint32_t (*)(nw_interface_t))orig_dlsym(libnetworkHandle, "nw_interface_get_index");
            }
            if (!orig_nw_interface_get_index) {
                orig_nw_interface_get_index = (uint32_t (*)(nw_interface_t))orig_dlsym(RTLD_DEFAULT, "nw_interface_get_index");
            }
        }

        if (!orig_nw_interface_get_type) {
            if (libnetworkHandle) {
                orig_nw_interface_get_type = (nw_interface_type_t (*)(nw_interface_t))orig_dlsym(libnetworkHandle, "nw_interface_get_type");
            }
            if (!orig_nw_interface_get_type) {
                orig_nw_interface_get_type = (nw_interface_type_t (*)(nw_interface_t))orig_dlsym(RTLD_DEFAULT, "nw_interface_get_type");
            }
        }
        ...
        if (!orig_nw_interface_create_with_index_and_name) {
            if (libnetworkHandle) {
                orig_nw_interface_create_with_index_and_name =
                    (nw_interface_t (*)(unsigned int, const char *))orig_dlsym(libnetworkHandle, "nw_interface_create_with_index_and_name");
            }
            if (!orig_nw_interface_create_with_index_and_name) {
                orig_nw_interface_create_with_index_and_name =
                    (nw_interface_t (*)(unsigned int, const char *))orig_dlsym(RTLD_DEFAULT, "nw_interface_create_with_index_and_name");
            }
        }
    }
    ...
    if (!dscEnumerate) {
        dscEnumerate = (void (*)(nw_path_t, nw_path_enumerate_interfaces_block_t))lc_findNetworkDSCSymbol("__nw_path_enumerate_interfaces");
    }
    ...
    if (!dscCopyWithGeneration) {
        dscCopyWithGeneration = (nw_interface_t (*)(void *, unsigned int, unsigned int))lc_findNetworkDSCSymbol("__nw_path_copy_interface_with_generation");
    }
    const char *(*dscGetName)(nw_interface_t) =
        (const char *(*)(nw_interface_t))lc_findNetworkDSCSymbol("_nw_interface_get_name");
    if (!dscGetName) {
        dscGetName = (const char *(*)(nw_interface_t))lc_findNetworkDSCSymbol("__nw_interface_get_name");
    }
    uint32_t (*dscGetIndex)(nw_interface_t) =
        (uint32_t (*)(nw_interface_t))lc_findNetworkDSCSymbol("_nw_interface_get_index");
    if (!dscGetIndex) {
        dscGetIndex = (uint32_t (*)(nw_interface_t))lc_findNetworkDSCSymbol("__nw_interface_get_index");
    }
    ...
    if (!dscGetType) {
        dscGetType = (nw_interface_type_t (*)(nw_interface_t))lc_findNetworkDSCSymbol("__nw_interface_get_type");
    }
    ...
    if (!dscCreateWithName) {
        dscCreateWithName = (nw_interface_t (*)(const char *))lc_findNetworkDSCSymbol("__nw_interface_create_with_name");
    }
    ...
    if (!dscCreateWithIndexAndName) {
        dscCreateWithIndexAndName = (nw_interface_t (*)(unsigned int, const char *))lc_findNetworkDSCSymbol("__nw_interface_create_with_index_and_name");
    }
    ...
    if (dscGetIndex) {
        orig_nw_interface_get_index = dscGetIndex;
    }
    if (dscGetType) {
        orig_nw_interface_get_type = dscGetType;
    }
    if (dscCreateWithName) {
        orig_nw_interface_create_with_name = dscCreateWithName;
    }
    if (dscCreateWithIndexAndName) {
        orig_nw_interface_create_with_index_and_name = dscCreateWithIndexAndName;
    }

    static BOOL loggedSymbolResolution = NO;
    if (!loggedSymbolResolution) {
        loggedSymbolResolution = YES;
        NSLog(@"[LC] 🌐 network symbols resolved: nw_path_enumerate_interfaces=%p nw_path_copy_interface_with_generation=%p nw_interface_get_name=%p nw_interface_get_index=%p nw_interface_get_type=%p nw_interface_create_with_name=%p nw_interface_create_with_index_and_name=%p",
              orig_nw_path_enumerate_interfaces,
              orig_nw_path_copy_interface_with_generation,
              orig_nw_interface_get_name,
              orig_nw_interface_get_index,
...
static NSString *lc_getInterfaceNameFromObject(id interfaceObj) {
    ...
    @try {
        ...
        } else if ([interfaceObj respondsToSelector:NSSelectorFromString(@"interfaceName")]) {
            name = ((id (*)(id, SEL))objc_msgSend)(interfaceObj, NSSelectorFromString(@"interfaceName"));
        }
        if (![name isKindOfClass:[NSString class]]) {
            ...
            if ([kvcName isKindOfClass:[NSString class]]) {
                ...
                id kvcInterfaceName = [interfaceObj valueForKey:@"interfaceName"];
                if ([kvcInterfaceName isKindOfClass:[NSString class]]) {
                    name = kvcInterfaceName;
                }
            }
        }
...
static uint32_t lc_getInterfaceIndexFromObject(id interfaceObj) {
    if (!interfaceObj) {
        return 0;
    }
    ...
    @try {
        ...
        if ([interfaceObj respondsToSelector:interfaceIndexSEL]) {
            NSInteger value = ((NSInteger (*)(id, SEL))objc_msgSend)(interfaceObj, interfaceIndexSEL);
            if (value > 0) {
                index = (uint32_t)value;
            }
        }
        if (index == 0 && [interfaceObj respondsToSelector:@selector(index)]) {
            NSInteger value = ((NSInteger (*)(id, SEL))objc_msgSend)(interfaceObj, @selector(index));
            if (value > 0) {
                index = (uint32_t)value;
            }
        }
        if (index == 0) {
            id kvcInterfaceIndex = [interfaceObj valueForKey:@"interfaceIndex"];
            if ([kvcInterfaceIndex respondsToSelector:@selector(unsignedIntValue)]) {
                index = (uint32_t)[kvcInterfaceIndex unsignedIntValue];
            } else {
                id kvcIndex = [interfaceObj valueForKey:@"index"];
                if ([kvcIndex respondsToSelector:@selector(unsignedIntValue)]) {
                    index = (uint32_t)[kvcIndex unsignedIntValue];
                }
            }
        }
    } @catch (__unused NSException *e) {
...
static NSString *hook_NWInterface_interfaceName_objc(id self, SEL _cmd) {
    if (!orig_NWInterface_interfaceName_objc) {
        return @"en0";
    }

    NSString *name = orig_NWInterface_interfaceName_objc(self, _cmd);
    if (!shouldFilterVPNInterfaceName(name)) {
        return name;
    }
    ...
    if (!loggedHookHit) {
        loggedHookHit = YES;
        NSLog(@"[LC] ✅ hook hit: -[NWInterface interfaceName]");
    }

    NSLog(@"[LC] 🎭 NWInterface.interfaceName - filtering VPN interface: %@", name);
    return @"en0";
}

static NSInteger hook_NWInterface_index_objc(id self, SEL _cmd) {
    NSInteger index = orig_NWInterface_index_objc ? orig_NWInterface_index_objc(self, _cmd) : 0;
    if (index <= 0) {
        return index;
    }

    if (!lc_isFilteredInterfaceIndex((uint32_t)index)) {
        return index;
    }
    ...
    if (!loggedHookHit) {
        loggedHookHit = YES;
        NSLog(@"[LC] ✅ hook hit: -[NWInterface index]");
    }
    NSLog(@"[LC] 🎭 NWInterface.index - remapping VPN ifindex %ld -> %u",
          (long)index,
          safeIndex);
    return (NSInteger)safeIndex;
}

static NSInteger hook_NWInterface_interfaceIndex_objc(id self, SEL _cmd) {
    NSInteger index = orig_NWInterface_interfaceIndex_objc ? orig_NWInterface_interfaceIndex_objc(self, _cmd) : 0;
    if (index <= 0) {
        return index;
    }
    ...
    if (!loggedHookHit) {
        loggedHookHit = YES;
        NSLog(@"[LC] ✅ hook hit: -[NWInterface interfaceIndex]");
    }
    NSLog(@"[LC] 🎭 NWInterface.interfaceIndex - remapping VPN ifindex %ld -> %u",
          (long)index,
          safeIndex);
    return (NSInteger)safeIndex;
}
...
static NSInteger hook_NWInterface_type_objc(id self, SEL _cmd) {
    NSInteger type = orig_NWInterface_type_objc ? orig_NWInterface_type_objc(self, _cmd) : 0;
    NSString *name = lc_getInterfaceNameFromObject(self);
    uint32_t index = lc_getInterfaceIndexFromObject(self);
    if (!shouldFilterVPNInterfaceName(name) && !lc_isFilteredInterfaceIndex(index)) {
        return type;
    }
    ...
    if (!loggedHookHit) {
        loggedHookHit = YES;
        NSLog(@"[LC] ✅ hook hit: -[NWInterface type]");
    }
    // Match NWInterface.InterfaceType.wifi raw value.
    return 1;
}
...
static NSArray *hook_NWPath_interfaces_objc(id self, SEL _cmd) {
    if (!orig_NWPath_interfaces_objc) {
        return @[];
    }

    NSArray *interfaces = orig_NWPath_interfaces_objc(self, _cmd);
    ...
    if (!loggedHookHit) {
        loggedHookHit = YES;
        NSLog(@"[LC] ✅ hook hit: -[NWPath interfaces]");
    }
    ...
    for (id interfaceObj in interfaces) {
        ...
        uint32_t index = lc_getInterfaceIndexFromObject(interfaceObj);
        if (shouldFilterVPNInterfaceName(name) || lc_isFilteredInterfaceIndex(index)) {
            NSLog(@"[LC] 🎭 NWPath.interfaces - removing VPN interface: %@ (%u)",
                  name ?: @"(unknown)",
                  index);
            continue;
        }
...
static NSArray *hook_NWPath_availableInterfaces_objc(id self, SEL _cmd) {
    ...
    for (id interfaceObj in interfaces) {
        ...
        uint32_t index = lc_getInterfaceIndexFromObject(interfaceObj);
        if (shouldFilterVPNInterfaceName(name) || lc_isFilteredInterfaceIndex(index)) {
            NSLog(@"[LC] 🎭 NWPath.availableInterfaces - removing VPN interface: %@ (%u)",
                  name ?: @"(unknown)",
                  index);
            continue;
        }
...
static id lc_createSafeNWInterfaceObject(void) {
    Class nwInterfaceClass = objc_getClass("NWInterface");
    if (nwInterfaceClass) {
        id allocObj = ((id (*)(id, SEL))objc_msgSend)((id)nwInterfaceClass, @selector(alloc));
        if (allocObj) {
            SEL initWithInterfaceNameSEL = NSSelectorFromString(@"initWithInterfaceName:");
            if ([allocObj respondsToSelector:initWithInterfaceNameSEL]) {
                id safeObj = ((id (*)(id, SEL, NSString *))objc_msgSend)(allocObj,
                                                                          initWithInterfaceNameSEL,
                                                                          @"en0");
                if (safeObj) {
                    return safeObj;
                }
            }
            SEL initWithInterfaceIndexSEL = NSSelectorFromString(@"initWithInterfaceIndex:");
            if ([allocObj respondsToSelector:initWithInterfaceIndexSEL]) {
                uint32_t safeIndex = lc_findSafeInterfaceIndex();
                if (safeIndex != 0) {
                    id safeObj = ((id (*)(id, SEL, NSInteger))objc_msgSend)(allocObj,
                                                                             initWithInterfaceIndexSEL,
                                                                             (NSInteger)safeIndex);
                    if (safeObj) {
                        return safeObj;
                    }
    ...
    if (orig_nw_interface_create_with_name) {
        return (id)orig_nw_interface_create_with_name("en0");
    }
    return nil;
}

static id lc_sanitizeNWPathInterfaceObject(NSString *context, id interfaceObj) {
    if (!interfaceObj) {
        return interfaceObj;
    }
    ...
    NSLog(@"[LC] 🎭 %@ - sanitizing VPN interface: %@ (%u)",
          context ?: @"NWPath",
          name ?: @"(unknown)",
          index);
    id safe = lc_createSafeNWInterfaceObject();
    return safe ?: interfaceObj;
}

static id hook_NWPath_interface_objc(id self, SEL _cmd) {
    id interfaceObj = orig_NWPath_interface_objc ? orig_NWPath_interface_objc(self, _cmd) : nil;
    return lc_sanitizeNWPathInterfaceObject(@"NWPath.interface", interfaceObj);
}

static id hook_NWPath_connectedInterface_objc(id self, SEL _cmd) {
    id interfaceObj = orig_NWPath_connectedInterface_objc ? orig_NWPath_connectedInterface_objc(self, _cmd) : nil;
    return lc_sanitizeNWPathInterfaceObject(@"NWPath.connectedInterface", interfaceObj);
}

static id hook_NWPath_delegateInterface_objc(id self, SEL _cmd) {
    id interfaceObj = orig_NWPath_delegateInterface_objc ? orig_NWPath_delegateInterface_objc(self, _cmd) : nil;
    return lc_sanitizeNWPathInterfaceObject(@"NWPath.delegateInterface", interfaceObj);
}

static id hook_NWPath_scopedInterface_objc(id self, SEL _cmd) {
    id interfaceObj = orig_NWPath_scopedInterface_objc ? orig_NWPath_scopedInterface_objc(self, _cmd) : nil;
    return lc_sanitizeNWPathInterfaceObject(@"NWPath.scopedInterface", interfaceObj);
}

static id hook_NWPath_fallbackInterface_objc(id self, SEL _cmd) {
    id interfaceObj = orig_NWPath_fallbackInterface_objc ? orig_NWPath_fallbackInterface_objc(self, _cmd) : nil;
    return lc_sanitizeNWPathInterfaceObject(@"NWPath.fallbackInterface", interfaceObj);
}

static NSUInteger hook_NWPath_fallbackInterfaceIndex_objc(id self, SEL _cmd) {
    NSUInteger index = orig_NWPath_fallbackInterfaceIndex_objc ? orig_NWPath_fallbackInterfaceIndex_objc(self, _cmd) : 0;
    if (index == 0 || !lc_isFilteredInterfaceIndex((uint32_t)index)) {
        return index;
    }
    uint32_t safeIndex = lc_findSafeInterfaceIndex();
    if (safeIndex == 0) {
    ...
    NSLog(@"[LC] 🎭 NWPath.fallbackInterfaceIndex - remapping VPN ifindex %lu -> %u",
          (unsigned long)index,
          safeIndex);
...
static void lc_installNWObjCInterfaceHooks(void) {
    ...
    SEL interfacesSEL = NSSelectorFromString(@"interfaces");
    SEL interfaceSEL = NSSelectorFromString(@"interface");
    SEL connectedInterfaceSEL = NSSelectorFromString(@"connectedInterface");
    SEL delegateInterfaceSEL = NSSelectorFromString(@"delegateInterface");
    ...
    for (size_t i = 0; i < (sizeof(pathClassNames) / sizeof(pathClassNames[0])); i++) {
        if (orig_NWPath_interface_objc &&
            orig_NWPath_connectedInterface_objc &&
            orig_NWPath_delegateInterface_objc &&
            orig_NWPath_scopedInterface_objc &&
            orig_NWPath_fallbackInterface_objc &&
            orig_NWPath_fallbackInterfaceIndex_objc) {
            break;
        }
        ...
        if (!orig_NWPath_availableInterfaces_objc) {
            Method availableInterfacesMethod =
                class_getInstanceMethod(pathClass, availableInterfacesSEL);
        ...
        if (!orig_NWPath_interfaces_objc) {
            Method interfacesMethod = class_getInstanceMethod(pathClass, interfacesSEL);
            if (interfacesMethod) {
                orig_NWPath_interfaces_objc = (NSArray *(*)(id, SEL))
                    method_setImplementation(interfacesMethod, (IMP)hook_NWPath_interfaces_objc);
                NSLog(@"[LC] 🌐 installed ObjC hook for %s interfaces", pathClassNames[i]);
            }
        }
        if (!orig_NWPath_interface_objc) {
            Method method = class_getInstanceMethod(pathClass, interfaceSEL);
            if (method) {
                orig_NWPath_interface_objc = (id (*)(id, SEL))
                    method_setImplementation(method, (IMP)hook_NWPath_interface_objc);
                NSLog(@"[LC] 🌐 installed ObjC hook for %s interface", pathClassNames[i]);
            }
        }
        if (!orig_NWPath_connectedInterface_objc) {
            Method method = class_getInstanceMethod(pathClass, connectedInterfaceSEL);
            if (method) {
                orig_NWPath_connectedInterface_objc = (id (*)(id, SEL))
                    method_setImplementation(method, (IMP)hook_NWPath_connectedInterface_objc);
                NSLog(@"[LC] 🌐 installed ObjC hook for %s connectedInterface", pathClassNames[i]);
            }
        }
        if (!orig_NWPath_delegateInterface_objc) {
            Method method = class_getInstanceMethod(pathClass, delegateInterfaceSEL);
            if (method) {
                orig_NWPath_delegateInterface_objc = (id (*)(id, SEL))
                    method_setImplementation(method, (IMP)hook_NWPath_delegateInterface_objc);
                NSLog(@"[LC] 🌐 installed ObjC hook for %s delegateInterface", pathClassNames[i]);
            }
        }
        if (!orig_NWPath_scopedInterface_objc) {
            Method method = class_getInstanceMethod(pathClass, scopedInterfaceSEL);
            if (method) {
                orig_NWPath_scopedInterface_objc = (id (*)(id, SEL))
                    method_setImplementation(method, (IMP)hook_NWPath_scopedInterface_objc);
                NSLog(@"[LC] 🌐 installed ObjC hook for %s scopedInterface", pathClassNames[i]);
            }
        }
        if (!orig_NWPath_fallbackInterface_objc) {
            Method method = class_getInstanceMethod(pathClass, fallbackInterfaceSEL);
            if (method) {
                orig_NWPath_fallbackInterface_objc = (id (*)(id, SEL))
                    method_setImplementation(method, (IMP)hook_NWPath_fallbackInterface_objc);
                NSLog(@"[LC] 🌐 installed ObjC hook for %s fallbackInterface", pathClassNames[i]);
            }
        }
        if (!orig_NWPath_fallbackInterfaceIndex_objc) {
            Method method = class_getInstanceMethod(pathClass, fallbackInterfaceIndexSEL);
            if (method) {
                orig_NWPath_fallbackInterfaceIndex_objc = (NSUInteger (*)(id, SEL))
                    method_setImplementation(method, (IMP)hook_NWPath_fallbackInterfaceIndex_objc);
    ...
    for (size_t i = 0; i < (sizeof(interfaceClassNames) / sizeof(interfaceClassNames[0])); i++) {
        ...
        if (!orig_NWInterface_interfaceName_objc) {
            Method interfaceNameMethod = class_getInstanceMethod(interfaceClass, interfaceNameSEL);
            if (interfaceNameMethod) {
                orig_NWInterface_interfaceName_objc = (NSString *(*)(id, SEL))
                    method_setImplementation(interfaceNameMethod, (IMP)hook_NWInterface_interfaceName_objc);
                NSLog(@"[LC] 🌐 installed ObjC hook for %s interfaceName", interfaceClassNames[i]);
            }
        }
        if (!orig_NWInterface_index_objc) {
            Method indexMethod = class_getInstanceMethod(interfaceClass, @selector(index));
            if (indexMethod) {
                orig_NWInterface_index_objc = (NSInteger (*)(id, SEL))
                    method_setImplementation(indexMethod, (IMP)hook_NWInterface_index_objc);
                NSLog(@"[LC] 🌐 installed ObjC hook for %s index", interfaceClassNames[i]);
            }
        }
        if (!orig_NWInterface_interfaceIndex_objc) {
            Method interfaceIndexMethod = class_getInstanceMethod(interfaceClass, interfaceIndexSEL);
            if (interfaceIndexMethod) {
                orig_NWInterface_interfaceIndex_objc = (NSInteger (*)(id, SEL))
                    method_setImplementation(interfaceIndexMethod, (IMP)hook_NWInterface_interfaceIndex_objc);
                NSLog(@"[LC] 🌐 installed ObjC hook for %s interfaceIndex", interfaceClassNames[i]);
            }
        }
        if (!orig_NWInterface_type_objc) {
            Method typeMethod = class_getInstanceMethod(interfaceClass, @selector(type));
            if (typeMethod) {
                orig_NWInterface_type_objc = (NSInteger (*)(id, SEL))
                    method_setImplementation(typeMethod, (IMP)hook_NWInterface_type_objc);
                NSLog(@"[LC] 🌐 installed ObjC hook for %s type", interfaceClassNames[i]);
            }
        }
        ...
        if ((orig_NWInterface_name_objc || orig_NWInterface_interfaceName_objc) &&
            orig_NWInterface_description_objc &&
            orig_NWInterface_debugDescription_objc &&
            (orig_NWInterface_index_objc || orig_NWInterface_interfaceIndex_objc) &&
            orig_NWInterface_type_objc) {
            break;
        }
    }

    BOOL hasPathHook = (orig_NWPath_availableInterfaces_objc != NULL) ||
                       (orig_NWPath_interfaces_objc != NULL) ||
                       (orig_NWPath_interface_objc != NULL) ||
                       (orig_NWPath_connectedInterface_objc != NULL) ||
                       (orig_NWPath_delegateInterface_objc != NULL) ||
                       (orig_NWPath_scopedInterface_objc != NULL) ||
                       (orig_NWPath_fallbackInterface_objc != NULL) ||
...
static void lc_networkImageAddedCallback(const struct mach_header *mh,
    ...
    Dl_info imageInfo = {0};
    if (dladdr(mh, &imageInfo) != 0) {
        imageName = imageInfo.dli_fname;
    }
    if (imageName && strstr(imageName, "libswiftNetwork.dylib") != NULL) {
...
static int hook_necp_client_action(int necp_fd,
    ...
    if (result >= 0 &&
        ...
        output_buffer &&
        output_buffer_size >= IFXNAMSIZ) {
        char *returnedName = (char *)output_buffer;
        if (shouldFilterVPNInterfaceNameCStr(returnedName)) {
            char oldName[IFXNAMSIZ + 1] = {0};
            strlcpy(oldName, returnedName, sizeof(oldName));
            strlcpy(returnedName, "en0", IFXNAMSIZ);
            NSLog(@"[LC] 🎭 necp_client_action - sanitized copy_interface name-only response %s -> en0",
                  oldName);
        }
    }
...
static uint32_t hook_nw_interface_get_index(nw_interface_t interface) {
    uint32_t index = orig_nw_interface_get_index ? orig_nw_interface_get_index(interface) : 0;
    if (index == 0) {
        return index;
    }

    const char *name = orig_nw_interface_get_name ? orig_nw_interface_get_name(interface) : NULL;
    BOOL filtered = shouldFilterVPNInterfaceNameCStr(name);
    if (!filtered) {
        filtered = lc_isFilteredInterfaceIndex(index);
    }
    if (!filtered) {
        return index;
    }
    ...
    NSLog(@"[LC] 🎭 nw_interface_get_index - remapping VPN interface index %u (%s) -> %u",
          index,
          name ? name : "(unknown)",
          safeIndex);
    return safeIndex;
...
static nw_interface_type_t hook_nw_interface_get_type(nw_interface_t interface) {
    nw_interface_type_t type = orig_nw_interface_get_type ? orig_nw_interface_get_type(interface) : nw_interface_type_other;

    const char *name = orig_nw_interface_get_name ? orig_nw_interface_get_name(interface) : NULL;
    BOOL filtered = shouldFilterVPNInterfaceNameCStr(name);
    if (!filtered && orig_nw_interface_get_index) {
        uint32_t index = orig_nw_interface_get_index(interface);
        filtered = lc_isFilteredInterfaceIndex(index);
    }
    if (!filtered) {
        return type;
    ...
    NSLog(@"[LC] 🎭 nw_interface_get_type - remapping VPN interface type (%s) -> wifi",
          name ? name : "(unknown)");
...
static nw_interface_t hook_nw_interface_create_with_name(const char *name) {
    ...
    if (shouldFilterVPNInterfaceNameCStr(name)) {
        NSLog(@"[LC] 🎭 nw_interface_create_with_name - remapping VPN interface: %s -> en0",
              name ? name : "(null)");
        return orig_nw_interface_create_with_name("en0");
    }
    return orig_nw_interface_create_with_name(name);
}
...
static nw_interface_t hook_nw_interface_create_with_index_and_name(unsigned int interface_index, const char *name) {
    ...
    if (!shouldFilterVPNInterfaceNameCStr(name)) {
        return orig_nw_interface_create_with_index_and_name(interface_index, name);
    }

    uint32_t safeIndex = lc_findSafeInterfaceIndex();
    if (safeIndex == 0) {
        safeIndex = interface_index;
    }
    NSLog(@"[LC] 🎭 nw_interface_create_with_index_and_name - remapping VPN interface %s (%u) -> en0 (%u)",
          name,
          interface_index,
          safeIndex);
    return orig_nw_interface_create_with_index_and_name(safeIndex, "en0");
}

static BOOL lc_isFilteredNWInterface(nw_interface_t interface,
                                     const char **resolvedNameOut,
                                     uint32_t *resolvedIndexOut) {
    if (resolvedNameOut) {
    ...
    const char *name = orig_nw_interface_get_name ? orig_nw_interface_get_name(interface) : NULL;
    if (resolvedNameOut) {
        *resolvedNameOut = name;
    }
    if (shouldFilterVPNInterfaceNameCStr(name)) {
        return YES;
    }

    uint32_t interfaceIndex = orig_nw_interface_get_index ? orig_nw_interface_get_index(interface) : 0;
    if (resolvedIndexOut) {
        *resolvedIndexOut = interfaceIndex;
    }

    if (interfaceIndex != 0) {
        char indexName[IFNAMSIZ] = {0};
        if (if_indextoname(interfaceIndex, indexName) &&
            shouldFilterVPNInterfaceNameCStr(indexName)) {
            if (resolvedNameOut && !*resolvedNameOut) {
                *resolvedNameOut = "utun";
            }
            return YES;
        }
    }
...
static nw_interface_t lc_sanitizePrivatePathInterface(const char *context, nw_interface_t interface) {
    ...
    if (!orig_nw_interface_get_name || !orig_nw_interface_get_index || !orig_nw_interface_create_with_name) {
        lc_resolveNetworkSymbolPointers();
    }

    const char *name = NULL;
    uint32_t interfaceIndex = 0;
    if (!lc_isFilteredNWInterface(interface, &name, &interfaceIndex)) {
        return interface;
    }

    NSLog(@"[LC] 🎭 %s - filtering VPN interface: name=%s index=%u",
          context,
          name ? name : "(unknown)",
          interfaceIndex);
...
static BOOL lc_shouldEnableSwiftNetworkImportRebind(void) {
    const char *enableFlag = getenv("LC_ENABLE_SWIFTNETWORK_IMPORT_REBIND");
...
static void hook_nw_path_enumerate_interfaces(nw_path_t path,
    ...
    orig_nw_path_enumerate_interfaces(path, ^bool(nw_interface_t interface) {
        ...
        uint32_t interfaceIndex = 0;
        if (lc_isFilteredNWInterface(interface, &name, &interfaceIndex)) {
            NSLog(@"[LC] 🎭 nw_path_enumerate_interfaces - filtering VPN interface: name=%s index=%u",
                  name ? name : "(unknown)",
                  interfaceIndex);
            return true;
        }
...
static void setupNetworkFrameworkLowLevelHooks(void) {
    ...
    if (lc_shouldEnableSwiftNetworkImportRebind()) {
        ...
        static BOOL loggedSwiftNetworkOptOut = NO;
        if (!loggedSwiftNetworkOptOut) {
            loggedSwiftNetworkOptOut = YES;
            NSLog(@"[LC] 🌐 swiftNetwork import rebinding disabled by default (set LC_ENABLE_SWIFTNETWORK_IMPORT_REBIND=1 to enable)");
        }
    }
    lc_resolveNetworkSymbolPointers();
    ...
    if (!orig_nw_path_enumerate_interfaces &&
        ...
        // Continue anyway; if_nametoindex + necp hooks still apply.
    }

    // Essential NW hooks only.
    struct rebinding nwRebindings[16];
    size_t nwRebindingCount = 0;
    if (orig_nw_interface_get_name) {
        ...
        nwRebindings[nwRebindingCount++] =
            (struct rebinding){"_nw_interface_get_name",
                               (void *)hook_nw_interface_get_name,
                               (void **)&orig_nw_interface_get_name};
    }
    if (orig_nw_interface_get_index) {
        nwRebindings[nwRebindingCount++] =
            (struct rebinding){"nw_interface_get_index",
                               (void *)hook_nw_interface_get_index,
                               (void **)&orig_nw_interface_get_index};
        nwRebindings[nwRebindingCount++] =
            (struct rebinding){"_nw_interface_get_index",
                               (void *)hook_nw_interface_get_index,
                               (void **)&orig_nw_interface_get_index};
    }
    if (orig_nw_interface_get_type) {
        nwRebindings[nwRebindingCount++] =
            (struct rebinding){"nw_interface_get_type",
                               (void *)hook_nw_interface_get_type,
                               (void **)&orig_nw_interface_get_type};
        nwRebindings[nwRebindingCount++] =
            (struct rebinding){"_nw_interface_get_type",
                               (void *)hook_nw_interface_get_type,
                               (void **)&orig_nw_interface_get_type};
    }
    if (orig_nw_path_enumerate_interfaces) {
        ...
        nwRebindings[nwRebindingCount++] =
            (struct rebinding){"_nw_path_enumerate_interfaces",
                               (void *)hook_nw_path_enumerate_interfaces,
                               (void **)&orig_nw_path_enumerate_interfaces};
    }
    if (orig_nw_path_copy_interface_with_generation) {
        ...
        nwRebindings[nwRebindingCount++] =
            (struct rebinding){"_nw_path_copy_interface_with_generation",
                               (void *)hook_nw_path_copy_interface_with_generation,
                               (void **)&orig_nw_path_copy_interface_with_generation};
    }
    if (orig_nw_interface_create_with_name) {
        nwRebindings[nwRebindingCount++] =
            (struct rebinding){"nw_interface_create_with_name",
                               (void *)hook_nw_interface_create_with_name,
                               (void **)&orig_nw_interface_create_with_name};
    }
    if (orig_nw_interface_create_with_index_and_name) {
        nwRebindings[nwRebindingCount++] =
            (struct rebinding){"nw_interface_create_with_index_and_name",
                               (void *)hook_nw_interface_create_with_index_and_name,
                               (void **)&orig_nw_interface_create_with_index_and_name};
    }

    if (nwRebindingCount > 0) {
        rebind_symbols(nwRebindings, nwRebindingCount);
        NSLog(@"[LC] 🌐 fishhook rebound nw symbols: get_name=%p get_index=%p get_type=%p enumerate_interfaces=%p copy_interface_with_generation=%p create_with_name=%p create_with_index_and_name=%p",
              orig_nw_interface_get_name,
              orig_nw_interface_get_index,
              orig_nw_interface_get_type,
              orig_nw_path_enumerate_interfaces,
